...
- **Thumbnail preview follows Tiles.** The separate "N cells" stepper (`cadThumbCells`/`cadThumbZoomStep`) is **removed**; `cadUpdateThumbPreview` renders `cadPatMacro` tiles (`patMacroForTiles`) at natural fit and shows a read-only `N × N` caption. Saved `thumbCells:cadPatMacro` so the gallery card matches the preview.
