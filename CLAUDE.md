...
- **GIF export** (`downloadGIF` → `_buildGIF`): captures the open pattern's animation (48 frames sampled 0→TOTAL) off `cv` at zoom=1, median-cut to ≤256 colours (`_medianCut`), encodes a looping GIF89a with a faithful Kevin-Weiner LZW (`_lzwEncode`). Fully self-contained (no library/worker). Restores the live zoom/step after capturing.
