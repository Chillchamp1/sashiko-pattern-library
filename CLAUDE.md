...
- **Comments dropdown** (`#commentBar` / `toggleCommentMenu`, just below Download): a **💬 Comments ▾** popover with a scrollable list + a handle input (`#cmHandle`, persisted to `localStorage.sashiko_handle`) and comment textarea (`#cmText`) → **Post** (`postComment`). Stored per-pattern in Firestore subcollection **`patterns/{id}/comments/{cid}`** = `{id,handle,text,created,uid}` (functions `_fetchCommentsFromFirestore`/`_saveCommentToFirestore`/`renderCommentList`/`_loadComments` in experimental.js; `_resetCommentBar` called from `loadPattern`). **TEXT ONLY by design** — photo upload would need **Firebase Storage** (paid Blaze plan; the free Spark plan no longer provisions a bucket, and images don't fit Firestore's 1 MB doc limit), so photos are deferred until Storage is enabled. **Requires a `firestore.rules` redeploy** (new `match /comments/{cid}`: public read, signed-in create with text≤500/handle≤40, admin-only update/delete) — until deployed, reads/writes are denied.
