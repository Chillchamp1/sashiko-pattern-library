...
- **Anonymous auth:** `_initFirebase` loads `firebase-auth-compat`, signs in anonymously, and tracks `_authUid` (resolved promise `_authReady`, awaited by `_awaitAuth` with a 4 s cap). `_pushToFirestore`/`_deleteFromFirestore` await it and stamp `creatorId = _authUid`. Degrades gracefully if anonymous auth is off (writes fall back to the local id; reads are public). `window.sashikoMyUid()` prints the uid for the admin allowlist.
- **`firestore.rules`** (deploy via console paste or `firebase deploy --only firestore:rules`; also enable Console → Authentication → **Anonymous**): public read; `create` requires `creatorId == auth.uid`; `update` (edit/publish/**tombstone**) requires creator-or-`isAdmin()`; **`delete: if false`** (safe because deletes are tombstone *writes*, so the collection can't be wiped); size/shape validation. Admin = uids listed in `isAdmin()` (paste from `sashikoMyUid()`).
- **The client-side admin password `'111'` is cosmetic** (it's in the public HTML). Real admin/creator enforcement is the rules. Patterns created before anonymous auth (legacy `creatorId`) become admin-editable only.
