...
- **PDF export** (`downloadPDF` → `_buildPDF`): a printable **single A4 page** for a custom pattern. Layout: two equal-size main windows side by side — **Pattern** (the realistic stitch view with the **selected fabric** + dot grid, via `_pdfStitchWindow` reusing `_drawFabric`/`_cadDrawStitchGrid`/`_galStitchScene`/`_cadDrawStitch`, incl. the fabric-aware default yarn) and **Drafting lines** (`_pdfDraftWindow`: the **same content the gallery Draft toggle shows, but as lines** — the dot grid + `_galDraftShapes()` ruler-extended straight guides + every arc as its **full pre-drawn circle** (weak) + the stitch-path lines **highlighted** on top; *not* the individual stitches) — then, only when the pattern has **≥2 families**, a row/grid of smaller **pass windows** (`_pdfPassWindow`, one per family, that family's geometry as lines in a print-darkened family colour via `_pdfInk`). `_pdfPassLayout` picks columns/cell so all passes + labels fit the page (cell capped so passes stay smaller than the mains). Each window is rendered to an offscreen canvas (drawing in anim coords 0..SIZE, supersampled ~3×) and embedded as a **JPEG image XObject** (DCTDecode). Non-custom patterns fall back to a single centred snapshot of the current view. The PDF is written by hand in `_pdfSerialize` (objects: Catalog·Pages·Page·Contents·Helvetica font·N images; correct xref offsets; titles ASCII-folded via `_pdfAscii` for WinAnsi/Helvetica) — **fully self-contained, no library**, same philosophy as the GIF encoder. Verified across 1/2/4/12-family patterns + Seigaiha/Kamon (arc circles in the draft window) and every fabric (valid `%PDF`…`%%EOF`, all windows on-page).
- **Fabric picker** (gallery viewer, `#galFabBtn` → `galToggleFabric`, popover `#galFabricPop`): a curated set of sashiko cloths `SASHIKO_FABRICS` (cad-engine.js: **Indigo, Midnight, Black, Slate grey, Natural, Kakishibu**), each baked once into a 500×500 woven texture (`_bakeFabric`, cached in `_fabricBufs`; twill sheen + fibre speckle + vignette tuned to cloth lightness). `_drawFabric(x,id,w,h)` replaces the hard-coded denim in the **gallery viewer + PDF** only — the CAD editor and thumbnails keep the default indigo `_cadDrawDenim`. State `galFabric` **persists across pattern loads** (an aesthetic preference, unlike per-family thread colours). On a **light** cloth (`_fabricById(galFabric).light`, e.g. Natural) the **default yarn** goes dark indigo (`_galDefaultYarn`, so stitches never vanish) and the dot grid draws **dark** (`_cadDrawStitchGrid(...,dark)`); `_galDrawDraft` guides likewise. Fabric is a live view setting, **not saved** to the pattern.
- **Thread-colour panel is one panel, no tabs** (`galBuildSwatches`): palettes shown as **labelled sections** — `Default` (off-white reset), `Olympus` (the 40 `OLYMPUS_SASHIKO`), `Pastel` (`GAL_PASTEL`) — built from `_galPaletteSections()` (extensible: add a brand → new section). The old palette **tab buttons** (`galSetPalette`, `#galPalSashiko`/`#galPalPastel`) are **removed**. Family chips + the live name caption (`#galSwName`) + `↺ All white` reset stay. `galToggleColours`/`galToggleFabric`/`galToggleAdv` share `_galClosePops` so only one popover is open at a time.
