...
- **Left-panel toolbar redesign (2026-07-23):** row 1 = the modal **drawing tools** (✏ Line — renamed from "Draw" — ⭕ Arc, ⬭ Ellipse, ✂ Cut, 🎨 Color; raised buttons with `.on` state) · divider · **↩ Undo** right beside them; row 2 (after the `sep2` break) = **Move cluster** (↑↓←→, ↻ 45°, ◇ 45°, ◆ Diamond) · 🖼 Image · divider (`sep3`) · **🔍 Reset view · 🗑 Clear**. One-shot commands carry `.cad-act` (flat/transparent until hover, styles.css) so tools and actions read differently; **Clear is no longer red** (it's undoable — `cadClear` pushes history). All buttons have explanatory `title` tooltips. `cad-toolbar.json` matches this order (incl. `ellipse`/`bgimg`/`sep3`). `_cadTbApply` now appends dids UNKNOWN to the saved layout at the END of the zone (they used to strand at the FRONT — how the new Ellipse button ended up far from Arc on the live site); `_cadApplyToolbarCache` discards a pre-ellipse admin draft (stale drafts would re-scramble).
- **Tool clicks never fail silently (2026-07-23):** the draw tool's hidden "recolor shortcut" (selected fam swatch + click near an existing line hijacked the click to recolor) is REMOVED — draw always draws, recoloring is the 🎨 tool. Arc/ellipse: a finishing click with ~zero sweep used to silently DISCARD the whole shape — now it's ignored (state kept) and every invalid click (zero radius, ellipse second-radius click ON the axis line) sets a corrective message via `hintOverride` in the pointerdown handler. **`#cadArcHint` moved from the bottom of the CAD view to directly under the Draw canvas** so the multi-click tools' step labels are actually visible while drawing.
