...
- **PDF export** (`downloadPDF` → `_buildPDF`): a printable **single A4 page** for a custom pattern. Layout: two equal-size main windows side by side — **Pattern** (the realistic denim+stitch view with the dot grid, via `_pdfStitchWindow` reusing `_cadDrawDenim`/`_cadDrawStitchGrid`/`_galStitchScene`/`_cadDrawStitch`) and **Drafting lines** (`_pdfDraftWindow`: same framing, dot grid + the whole pattern as *weak* thin lines) — then, only when the pattern has **≥2 families**, a row/grid of smaller **pass windows** (`_pdfPassWindow`, one per family, that family's geometry as lines in a print-darkened family colour via `_pdfInk`). `_pdfPassLayout` picks columns/cell so all passes + labels fit the page (cell capped so passes stay smaller than the mains). Each window is rendered to an offscreen canvas (drawing in anim coords 0..SIZE, supersampled ~3×) and embedded as a **JPEG image XObject** (DCTDecode). Non-custom patterns fall back to a single centred snapshot of the current view. The PDF is written by hand in `_pdfSerialize` (objects: Catalog·Pages·Page·Contents·Helvetica font·N images; correct xref offsets; titles ASCII-folded via `_pdfAscii` for WinAnsi/Helvetica) — **fully self-contained, no library**, same philosophy as the GIF encoder. Verified across 1/2/4/12-family patterns (valid `%PDF`…`%%EOF`, all windows on-page).
