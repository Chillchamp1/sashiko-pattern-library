...
- **Client admin UI is gated on `ADMIN_EMAILS`** (experimental.js — set it to the same address as the rules' `isAdmin()`). Anyone can *authenticate* with any Google account (that's how Google sign-in works), but `_isAdmin()` = signed-in email ∈ `ADMIN_EMAILS`, so non-admins don't see the admin UI; `_ensureAdmin` signs a non-allowed account back to anonymous with a message. The rules remain the real boundary. While `ADMIN_EMAILS` holds the placeholder it's "unconfigured" → any Google sign-in shows admin UI (so you're not locked out before setting it).
