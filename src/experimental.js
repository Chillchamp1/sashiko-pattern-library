...
let EXP_PATTERNS=[];
window.EXP_PATTERNS_ref=()=>EXP_PATTERNS;
...
function renderExpPattern(pat){
  ...
  const tileW=dU,tileH=dV;
  const isIso=pat.gridType==='isometric';
  let tileSize;
  if(isIso){tileSize=Math.min(SIZE/(tileW*2*COS30+4),SIZE/(tileH*2*SIN30+4))*1.5;}
  else{tileSize=Math.min(SIZE/(tileW+4),SIZE/(tileH+4))*1.5;}
  ...
  const over=Math.ceil(SIZE/Math.max(tileSize*tileW,tileSize*tileH,1))+2;
...
function expCardHTML(pat){
  return`<div class="pcard exp-card" data-id="${pat.id}" data-exp="1" onclick="openExpPattern('${pat.id}')">
...
function rebuildExpGallery(){
  document.querySelectorAll('.pcard.exp-card').forEach(c=>c.remove());
