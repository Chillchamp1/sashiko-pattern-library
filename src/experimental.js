...
window.editExpPattern=async function(idOrPat){
  ...
  // Restore the custom family colours + coloured-thread flag + fabric (community feature).
...
function _isKnownMode(m){return m==='default'||m==='continuous'||m==='contour'||m==='sequential'||_isV2Mode(m);}   // retired: smooth, fewer-jumps, rows2, rows2e
...
// Two selectable routing modes in a separate pipeline (buildExpPath dispatches here
...
//           the robust band-snake (_v2OrderFamily: gap-CLUSTERED bands with RANK parity,
//           CENTROID band coordinate, movement-type + 30° orientation bins, v1/NN fallback
//           candidates — most PREDICTABLE within 15% of the cheapest) instead of the
//           fragile min-gap pitch.
// (rows2 "1b · Rows v2 · ordered" and rows2e "1c · Rows v2 · efficient" were retired
//  2026-07-22 once no saved pattern used them; waves2 keeps their ordering machinery.)
...
function _isV2Mode(m){return m==='zigzag2'||m==='waves2';}
...
// (The 'efficient' variant — outright cheapest, mode rows2e — was retired 2026-07-22.)
function _v2OrderFamily(strokes){
  ...
  // The most predictable candidate within 15% of the cheapest score —
...
function buildExpPathV2(lines,famOrderOverride,mode,opts){
  ...
  // Only waves2 reaches this tail (rows2/rows2e retired 2026-07-22): contour waves
  // exactly as mode 3 builds them, ordered with the robust band-snake below.
  const famData=[];
  for(const[fi,segs]of famGroups){
    const strokes=buildContourStrokes(segs,120*Math.PI/180);
    const infos=_v2OrderFamily(strokes);
...
window.remixPattern=function(id){
  ...
  // Custom colours + fabric carry into the remix (visual identity) — they re-activate
...
window.showCAD=function(){
  ...
  cadFamColors={};cadStitchColors=false;cadFabric='indigo';cadFamColorOpen=null;   // fresh pattern = classic palette + denim
  cadInited=false;
  cadInit();
  window.scrollTo({top:0,behavior:'smooth'});
};
