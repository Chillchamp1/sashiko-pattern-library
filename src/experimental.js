...
function buildExpPathV2(lines,famOrderOverride,mode,opts){
  ...
  if(mode==='zigzag2'){
    ...
    // 878 apex-broken ones). The drawn family is recovered per segment for colouring.
    const SK=(a,b)=>{const k1=_v2VKey(a),k2=_v2VKey(b);return k1<k2?k1+'|'+k2:k2+'|'+k1;};
    const famOfSeg=new Map();
    lines.forEach(l=>famOfSeg.set(SK(l.start,l.end),l.fam||0));
    const ordered=_v2NN(buildStrokesForFamily(lines,Math.PI).map(p=>_v2StrokeInfo(p)).filter(s=>s.pts.length>=2),null);
    const path=[];let cur=null,lastFam=0;
    for(const s of ordered){
      let pts=s.pts;
      const closed=pts.length>=3&&Math.hypot(pts[0][0]-pts[pts.length-1][0],pts[0][1]-pts[pts.length-1][1])<1e-3;
      if(closed)pts=_rotateClosedEntryV2(pts,cur||pts[0],allowed);
      ...
      for(let k=0;k<pts.length-1;k++){
        const f=famOfSeg.get(SK(pts[k],pts[k+1]));
        if(f!==undefined)lastFam=f;
        path.push({start:pts[k],end:pts[k+1],jump:!!cur&&k===0,fam:lastFam});
      }
      cur=pts[pts.length-1];
    }
    return path;
