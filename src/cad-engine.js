...
// Ellipse tool state: 0=await center, 1=await first-axis point (sets rx + rotation),
// 2=await second radius (perpendicular distance), 3=sweeping (parameter space).
let cadEllState=0,cadEllCenter=null,cadEllAx1=null,cadEllRx=0,cadEllRy=0,cadEllRot=0;
let cadEllSweep=0,cadEllPrevAng=0,cadEllA1=0;
...
function cadDistToArc(p, arc){
  if(arc.ellipse)return _cadDistToEllipse(p,arc);
  if(cadGridType==='isometric'){
...
function cadLineArcIntersections(p1,p2,arc){
  if(arc.ellipse)return _cadLineEllipseIntersections(p1,p2,arc);
  if(cadGridType==='isometric'){
...
function cadArcArcIntersections(arcA, arcB){
  if(arcA.ellipse||arcB.ellipse)return _cadCurveCurveIntersections(arcA,arcB);
  if(cadGridType==='isometric'){
...
function cadBBox(){
  ...
  cadLines.forEach(l=>{
    if(l.arc){
      // Ellipse extremes don't sit at parameter multiples of π/2 (when rotated), so take
      // the bbox of the flattened polyline — which IS the stitched geometry anyway.
      if(l.ellipse){
        cadFlattenArc(l,48).forEach(s=>{
          mnu=Math.min(mnu,s.start[0],s.end[0]);mxu=Math.max(mxu,s.start[0],s.end[0]);
          mnv=Math.min(mnv,s.start[1],s.end[1]);mxv=Math.max(mxv,s.start[1],s.end[1]);
        });
        return;
      }
...
function cadFlattenArc(arc, nSegs){
  ...
  const result=[];
  // Elliptical arc → flatten via the parameter form, in grid space on both grid types
  // (no iso round-on-screen correction — that special case is circles-only).
  if(arc.ellipse){
    let prev=[...arc.start];
    for(let i=1;i<=segs;i++){
      const next=_cadEllPt(arc,a1+sweep*(i/segs));
...
// An elliptical arc is a generalised arc entry: {arc:true, ellipse:true, center,
// rx, ry, rot, a1, a2, start, end} (+ r = mean radius, a graceful fallback for any
// circle-only code path). a1/a2 are PARAMETER angles — φ in
//   p(φ) = center + R(rot)·(rx·cosφ, ry·sinφ)
// — not geometric angles. Parameters behave exactly like circle angles (periodic
// 2π, monotonic along the sweep), so ALL the existing sweep bookkeeping
// (cadAngleInArc, toSweep, sub-arc splitting, _dcClipArc's addA) works unchanged.
// Ellipses are drawn in grid (u,v) space on BOTH grid types — no iso round-on-screen
// correction (that special-case exists only for circles).
function _cadEllPt(el,phi){
  const co=Math.cos(el.rot||0), si=Math.sin(el.rot||0);
  const x=el.rx*Math.cos(phi), y=el.ry*Math.sin(phi);
  return[el.center[0]+x*co-y*si, el.center[1]+x*si+y*co];
}
function _cadEllParam(el,p){
  const co=Math.cos(el.rot||0), si=Math.sin(el.rot||0);
  const dx=p[0]-el.center[0], dy=p[1]-el.center[1];
  return Math.atan2((-dx*si+dy*co)/el.ry, (dx*co+dy*si)/el.rx);
}
// Point at parameter φ on any arc entry (circle or ellipse) — the generic form of
// center + r·(cosφ,sinφ) used by the cut/highlight/diamond-cut sub-arc builders.
function _cadArcPt(arc,phi){
  if(arc.ellipse)return _cadEllPt(arc,phi);
  return[arc.center[0]+arc.r*Math.cos(phi),arc.center[1]+arc.r*Math.sin(phi)];
}
// Build an ellipse arc from centre + axes + start parameter + accumulated sweep
// (the ellipse-tool analogue of cadGenArcSweep — any size, closes a full ellipse
// whichever way you sweep).
function cadGenEllipseSweep(center,rx,ry,rot,a1,sweep){
  if(rx<0.01||ry<0.01)return null;
  let sw=sweep;
  if(Math.abs(sw)>=2*Math.PI-0.12)sw=(sw>=0?1:-1)*2*Math.PI;   // nearly all the way → full ellipse
  if(Math.abs(sw)<0.02)return null;
  const el={arc:true,ellipse:true,center:[center[0],center[1]],rx,ry,rot,r:(rx+ry)/2,a1,a2:a1+sw};
  el.start=_cadEllPt(el,a1);el.end=_cadEllPt(el,el.a2);
  return el;
}
// Distance from a point to an ellipse arc (dense polyline; exact enough for the
// 15px hover threshold). Returns {dist, angle} with angle = parameter, like cadDistToArc.
function _cadDistToEllipse(p,el){
  const a1=el.a1;let sweep=el.a2-el.a1;
  if(sweep>=2*Math.PI-0.001)sweep=2*Math.PI;
  else if(sweep<=-2*Math.PI+0.001)sweep=-2*Math.PI;
  const n=Math.max(24,Math.round(Math.abs(sweep)/(2*Math.PI)*96));
  let prev=_cadEllPt(el,a1),bd=Infinity,bphi=a1;
  for(let i=1;i<=n;i++){
    const phi=a1+sweep*(i/n), pt=_cadEllPt(el,phi);
    const r=cadDistToSeg(p,prev,pt);
    if(r.dist<bd){bd=r.dist;bphi=a1+sweep*((i-1+r.t)/n);}
    prev=pt;
  }
  return{dist:bd,angle:bphi};
}
// EXACT line-segment ∩ ellipse arc via the unit-circle transform: W maps the ellipse
// to the unit circle (and lines to lines), and the angle in W-space IS the parameter.
function _cadLineEllipseIntersections(p1,p2,el){
  const co=Math.cos(el.rot||0), si=Math.sin(el.rot||0);
  const W=p=>{const dx=p[0]-el.center[0],dy=p[1]-el.center[1];return[(dx*co+dy*si)/el.rx,(-dx*si+dy*co)/el.ry];};
  const A=W(p1),B=W(p2);
  const dx=B[0]-A[0],dy=B[1]-A[1];
  const a=dx*dx+dy*dy,b=2*(A[0]*dx+A[1]*dy),c=A[0]*A[0]+A[1]*A[1]-1;
  const disc=b*b-4*a*c;
  if(disc<0||a<1e-12)return[];
  const sd=Math.sqrt(disc),out=[];
  for(const t of[(-b-sd)/(2*a),(-b+sd)/(2*a)]){
    if(t>0.001&&t<0.999){
      const phi=Math.atan2(A[1]+t*dy,A[0]+t*dx);
      if(cadAngleInArc(phi,el)!==null)
        out.push({p:[p1[0]+t*(p2[0]-p1[0]),p1[1]+t*(p2[1]-p1[1])],t,angle:phi});
    }
  }
  if(out.length>1&&Math.hypot(out[0].p[0]-out[1].p[0],out[0].p[1]-out[1].p[1])<0.01)out.length=1;
  return out;
}
// Curve ∩ curve when at least one side is an ellipse: intersect the two flattened
// chains (cadFlattenArc renders each curve exactly as drawn, incl. iso round circles)
// and recover each crossing's parameter on both curves. Chains are 72 segs — plenty
// for the cut tool's breakpoints.
function _cadCurveCurveIntersections(A,B){
  const fa=cadFlattenArc(A,72), fb=cadFlattenArc(B,72), out=[];
  const par=(arc,p)=>arc.ellipse?_cadEllParam(arc,p):Math.atan2(p[1]-arc.center[1],p[0]-arc.center[0]);
  for(const sa of fa)for(const sb of fb){
    const ix=cadLineInter(sa.start,sa.end,sb.start,sb.end);
    if(!ix)continue;
    const p=[ix[0],ix[1]];
    if(out.some(o=>Math.hypot(o.p[0]-p[0],o.p[1]-p[1])<0.01))continue;
    out.push({p,angleA:par(A,p),angleB:par(B,p)});
  }
  return out;
}
...
function cadArcLabel(){
  ...
  if(!el)return;
  if(cadTool==='ellipse'){
    const eLabels=['Click to place center','Click to set the first radius (its direction tilts the ellipse)','Click to set the second radius','Move to sweep the ellipse (either way), click to finish · all the way round = full ellipse'];
    el.textContent=cadEllState<4?eLabels[cadEllState]:'';
    return;
  }
  ...
  el.textContent=cadArcState<3?labels[cadArcState]:'';
}
// Ellipse tool state helpers: reset, the in-progress ellipse (live ry while choosing),
// and the perpendicular distance from the first-axis line (= the second radius).
function _cadEllReset(){cadEllState=0;cadEllCenter=null;cadEllAx1=null;cadEllRx=0;cadEllRy=0;cadEllRot=0;cadEllSweep=0;cadEllPrevAng=0;cadEllA1=0;}
function _cadEllProto(ry){return{center:cadEllCenter,rx:cadEllRx,ry:Math.max(ry!==undefined?ry:cadEllRy,0.01),rot:cadEllRot};}
function _cadEllPerpR(p){
  const si=Math.sin(cadEllRot), co=Math.cos(cadEllRot);
  return Math.abs(-(p[0]-cadEllCenter[0])*si+(p[1]-cadEllCenter[1])*co);
}
...
function cadDrawWorkspace(){
  ...
  if(cadTool==='ellipse'&&cadEllState===3&&cadEllCenter){
    const prevEl=cadGenEllipseSweep(cadEllCenter,cadEllRx,cadEllRy,cadEllRot,cadEllA1,cadEllSweep);
    if(prevEl)all.push({...prevEl,preview:true});
  }
  ...
  // Ellipse tool visuals
  if(cadTool==='ellipse'){
    const traceEll=(el,segsN)=>{
      const segs=cadFlattenArc(el,segsN||60);
    ...
    if(cadEllState===1&&cadEllCenter&&cadCur){
      // First axis being chosen: dashed centre→cursor guide (direction = tilt, length = rx).
      const pc=g2s(cadEllCenter[0],cadEllCenter[1]), pm=g2s(cadCur[0],cadCur[1]);
      x.strokeStyle='rgba(255,204,0,0.6)';x.setLineDash([5,5]);x.lineWidth=1;
      x.beginPath();x.moveTo(pc.x,pc.y);x.lineTo(pm.x,pm.y);x.stroke();
      x.setLineDash([]);
    }else if(cadEllState===2&&cadEllCenter&&cadEllAx1){
      // Second radius being chosen: dashed first-axis line + faint live full-ellipse hint.
      const pc=g2s(cadEllCenter[0],cadEllCenter[1]), pa=g2s(cadEllAx1[0],cadEllAx1[1]);
      x.strokeStyle='rgba(255,204,0,0.6)';x.setLineDash([5,5]);x.lineWidth=1;
      x.beginPath();x.moveTo(pc.x,pc.y);x.lineTo(pa.x,pa.y);x.stroke();
      if(cadCur){
        const ry=_cadEllPerpR(cadCur);
        if(ry>=0.01){
          const proto=_cadEllProto(ry);
          const hint={arc:true,ellipse:true,center:cadEllCenter,rx:cadEllRx,ry,rot:cadEllRot,
            a1:0,a2:2*Math.PI,start:_cadEllPt(proto,0),end:_cadEllPt(proto,0)};
          x.strokeStyle='rgba(0,255,204,0.35)';x.lineWidth=1.5;
          traceEll(hint);
        }
      }
      x.setLineDash([]);
    }else if(cadEllState===3&&cadEllCenter){
      // Sweeping: solid preview of the swept part + dashed guides to start and cursor.
      const prevEl=cadGenEllipseSweep(cadEllCenter,cadEllRx,cadEllRy,cadEllRot,cadEllA1,cadEllSweep);
      if(prevEl){
        x.strokeStyle='rgba(0,255,204,0.5)';x.lineWidth=3;
        traceEll(prevEl,80);
      }
      const pc=g2s(cadEllCenter[0],cadEllCenter[1]);
      const st=_cadEllPt(_cadEllProto(),cadEllA1), ps=g2s(st[0],st[1]);
      x.strokeStyle='rgba(255,204,0,0.4)';x.setLineDash([5,5]);x.lineWidth=1;
      x.beginPath();x.moveTo(pc.x,pc.y);x.lineTo(ps.x,ps.y);x.stroke();
      if(cadCur){const pe=g2s(cadCur[0],cadCur[1]);x.beginPath();x.moveTo(pc.x,pc.y);x.lineTo(pe.x,pe.y);x.stroke();}
      x.setLineDash([]);
    }
  }
  ...
  if(cadTool==='erase'&&cadHover){
    if(cadHover.isArc){
      ...
      if(ci>=0&&ci<all.length-1){
        ...
        const subArc={...arc,a1:ang0,a2:ang1,start:_cadArcPt(arc,ang0),end:_cadArcPt(arc,ang1)};
  ...
  if((cadTool==='draw'||cadTool==='arc'||cadTool==='ellipse')&&cadCur){
  ...
  if(cadTool==='ellipse'&&cadEllCenter){
    const pc=g2s(cadEllCenter[0],cadEllCenter[1]);
    x.fillStyle='rgba(255,204,0,0.8)';x.beginPath();x.arc(pc.x,pc.y,5,0,Math.PI*2);x.fill();
  }
  if(cadTool==='ellipse'&&cadEllAx1){
    const ps=g2s(cadEllAx1[0],cadEllAx1[1]);
    x.fillStyle='rgba(0,255,204,0.8)';x.beginPath();x.arc(ps.x,ps.y,5,0,Math.PI*2);x.fill();
  }
}
...
function cadDrawPattern(){
  ...
  const _renderAt=(ou,ov2)=>{all.forEach((l,li)=>{
    ...
    if(l.arc){
      // Transform arc center into tiled position (spread keeps ellipse fields)
      const tCenter=[l.center[0]-bbox.minU+ou, l.center[1]-bbox.minV+ov2];
      const tArc={...l,center:tCenter,
...
function cadUpdateAll(){
  ...
  const el=document.getElementById('cadArcHint');
  if(el){
    // The arc/ellipse tools own the hint line (their step-by-step labels persist here).
    const curveTool=cadTool==='arc'||cadTool==='ellipse';
    const red=cadFindRedundant();
    if(red.length && !curveTool)el.innerHTML='<span style=\"color:#ff5555\">'+red.length+' redundant line'+(red.length>1?'s':'')+' (red dashed) — excluded when saving</span>';
    else if(!curveTool)el.textContent='';
  }
}
...
window.cadSetTool=function(t){
  ...
  const eb=document.getElementById('cadBtnEllipse');if(eb)eb.classList.toggle('on',t==='ellipse');
...
window.cadRotate45=function(){
  ...
  cadLines=cadLines.map(l=>{
    if(l.arc&&l.ellipse){
      // Ellipse: rotate the centre and add 45° to the axis rotation; the parameter
      // interval a1/a2 is unchanged, so just recompute the endpoint coordinates.
      const el={...l,center:rot(l.center),rot:(l.rot||0)+Math.PI/4};
      el.start=_cadEllPt(el,el.a1);el.end=_cadEllPt(el,el.a2);
      return el;
    }
    if(l.arc){
...
function _dcClipArc(arc,rc){
  ...
  if(arc.ellipse){
    // Ellipse ∩ axis-aligned boundary line: the u- (or v-) coordinate at parameter φ is
    // A·cosφ + B·sinφ + c, so the crossings solve A·cosφ+B·sinφ=C → φ=atan2(B,A)±acos(C/√(A²+B²)).
    const co=Math.cos(arc.rot||0), si=Math.sin(arc.rot||0);
    const solve=(A,B,C)=>{
      const R=Math.hypot(A,B);
      if(R<1e-12||Math.abs(C)>R)return[];
      const al=Math.atan2(B,A), be=Math.acos(Math.max(-1,Math.min(1,C/R)));
      return[al-be,al+be];
    };
    for(const X of[rc.x0,rc.x1])solve(arc.rx*co,-arc.ry*si,X-cx).forEach(addA);
    for(const Y of[rc.y0,rc.y1])solve(arc.rx*si,arc.ry*co,Y-cy).forEach(addA);
  }else{
...
function _dcContainedArc(p,list){
  for(const l of list){
    if(!l.arc)continue;
    if(Math.abs(l.center[0]-p.center[0])>1e-3||Math.abs(l.center[1]-p.center[1])>1e-3)continue;
    if(!!l.ellipse!==!!p.ellipse)continue;
    if(l.ellipse){
      if(Math.abs(l.rx-p.rx)>1e-3||Math.abs(l.ry-p.ry)>1e-3||Math.abs((l.rot||0)-(p.rot||0))>1e-3)continue;
    }else if(Math.abs(l.r-p.r)>1e-3)continue;
...
function _cadLineToSaved(l, minU, minV){
  if(l.arc&&l.center!==undefined){
    ...
    if(l.ellipse){
      o.ellipse=true;
      o.rx=parseFloat(l.rx.toFixed(3));o.ry=parseFloat(l.ry.toFixed(3));
      o.rot=parseFloat((l.rot||0).toFixed(6));
    }
    return o;
  }
...
function _cadLineFromSaved(l, minU, minV){
  if(l.arc&&l.center!==undefined){
    ...
    const o={arc:true,center:c,r:l.r,a1:l.a1,a2:l.a2,start:[l.start[0]-minU,l.start[1]-minV],end:[l.end[0]-minU,l.end[1]-minV]};
...
function cadInit(){
  ...
  cv.addEventListener('pointerdown',e=>{
    ...
    if(cadTool==='draw'){
      ...
      if(cadEllState===0){cadEllCenter=[...cadCur];cadEllState=1;}
        ...
        if(ry>=0.01){
          cadEllRy=ry;
          // The sweep starts at the parameter under the cursor (mirrors the arc tool,
          // where the sweep starts at the radius click).
      ...
      if(cadHover.isArc){
        ...
        for(let i=0;i<all.length-1;i++){
          ...
          const p0=_cadArcPt(arc,ang0);
          const p1=_cadArcPt(arc,ang1);
          if(Math.hypot(p1[0]-p0[0],p1[1]-p0[1])>0.005){
            // Spread keeps the ellipse fields (rx/ry/rot) on the surviving sub-arcs.
            cadLines.push({...arc,center:[...arc.center],a1:ang0,a2:ang1,start:p0,end:p1});
          }
        }
      }else{
  ...
  cv.addEventListener('pointermove',e=>{
    ...
    const g=cadS2G(pos.x,pos.y,cadOX,cadOY,cadTileSize);
    if(cadTool==='draw'||cadTool==='arc'||cadTool==='ellipse')cadCur=cadSnapPoint(g.u,g.v);
    ...
    // Ellipse: same continuous accumulation, but in PARAMETER space (the mouse's parameter
    // on the chosen ellipse), so a full turn of the mouse closes a full ellipse either way.
    else if(cadTool==='ellipse'&&cadEllState===3&&cadEllCenter){
      const ang=_cadEllParam(_cadEllProto(),[g.u,g.v]);
      let d=ang-cadEllPrevAng; while(d>Math.PI)d-=2*Math.PI; while(d<=-Math.PI)d+=2*Math.PI;
      cadEllSweep=Math.max(-2*Math.PI,Math.min(2*Math.PI,cadEllSweep+d)); cadEllPrevAng=ang;
    }
