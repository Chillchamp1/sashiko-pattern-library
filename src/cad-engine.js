...
window.cadSaveToLibrary=function(){
  ...
  const lines=cleanLines.map(l=>({start:[parseFloat(l.start[0].toFixed(3)),parseFloat(l.start[1].toFixed(3))],end:[parseFloat(l.end[0].toFixed(3)),parseFloat(l.end[1].toFixed(3))]}));
...
window.cadPublishToLibrary=function(){
  ...
  const lines=cleanLines.map(l=>({start:[parseFloat(l.start[0].toFixed(3)),parseFloat(l.start[1].toFixed(3))],end:[parseFloat(l.end[0].toFixed(3)),parseFloat(l.end[1].toFixed(3))]}));
  const thumbnail=document.getElementById('cadCanvas').toDataURL('image/png');
  let pat={name,type:'exp',gridType:cadGridType,lines,bbox:{minU:bbox.minU,maxU:bbox.maxU,minV:bbox.minV,maxV:bbox.maxV},patMacro:cadPatMacro,thumbnail,createdAt:Date.now(),bboxRotated:cadBBoxRotated,famOrder:[...cadFamOrder],traditional:cadTraditional,published:true};
  if(cadEditId){
    const idx=EXP_PATTERNS.findIndex(p=>p.id===cadEditId);
    if(idx>=0){
...
window.cadTilePlay=function(){
  ...
  const lines=clean.map(l=>({start:[l.start[0],l.start[1]],end:[l.end[0],l.end[1]]}));
  const pat={type:'exp',gridType:cadGridType,lines,bbox:{minU:bbox.minU,maxU:bbox.maxU,minV:bbox.minV,maxV:bbox.maxV},patMacro:cadPatMacro,bboxRotated:cadBBoxRotated,famOrder:[...cadFamOrder]};
  pat.families=cadFamilies.filter((_,i)=>!redSet.has(i));
