...
let cadManualBBox=null,cadBBoxP1=null;
const CAD_MICRO=10;
...
function cadDrawWorkspace(){
  ...
  if(cadTool==='bbox'&&cadBBoxP1&&cadCur){
    const mu=Math.min(cadBBoxP1[0],cadCur[0]),Mu=Math.max(cadBBoxP1[0],cadCur[0]);
    const mv=Math.min(cadBBoxP1[1],cadCur[1]),Mv=Math.max(cadBBoxP1[1],cadCur[1]);
    const p1=cadG2S(mu,mv,cadOX,cadOY,cadTileSize),p2=cadG2S(Mu,mv,cadOX,cadOY,cadTileSize);
    const p3=cadG2S(Mu,Mv,cadOX,cadOY,cadTileSize),p4=cadG2S(mu,Mv,cadOX,cadOY,cadTileSize);
...
window.cadUpdateSettings=function(){
  ...
  cadZoom=1;cadPanX=0;cadPanY=0;cadManualBBox=null;cadBBoxP1=null;
...
window.cadSetTool=function(t){
  if(cadTool==='bbox'&&t==='bbox'&&cadManualBBox){cadManualBBox=null;cadBBoxP1=null;cadUpdateAll();return;}
  cadTool=t;
...
window.cadClear=function(){if(cadLines.length){cadHistory.push(JSON.parse(JSON.stringify(cadLines)));cadLines=[];cadManualBBox=null;cadBBoxP1=null;cadUpdateAll();}};
window.cadResetView=function(){cadZoom=1;cadPanX=0;cadPanY=0;cadManualBBox=null;cadBBoxP1=null;cadApplyView();cadBakeLeft();cadUpdateAll();};
...
window.cadClearManualBBox=function(){cadManualBBox=null;cadBBoxP1=null;cadUpdateAll();};
...
function cadInit(){
  ...
  cv.addEventListener('pointerdown',e=>{
    ...
    if(cadTool==='draw'){cadDrawing=true;cadStart=[cadCur[0],cadCur[1]];}
      ...
      if(!cadBBoxP1){cadBBoxP1=[cadCur[0],cadCur[1]];}
      else{
        const mu=Math.min(cadBBoxP1[0],cadCur[0]),Mu=Math.max(cadBBoxP1[0],cadCur[0]);
        const mv=Math.min(cadBBoxP1[1],cadCur[1]),Mv=Math.max(cadBBoxP1[1],cadCur[1]);
        cadManualBBox={minU:mu,maxU:Mu,minV:mv,maxV:Mv};cadBBoxP1=null;cadCur=null;
      }
    }
    cadUpdateAll();
  });
  cv.addEventListener('pointermove',e=>{
    ...
    else if(cadTool==='bbox'&&cadBBoxP1){cadCur=cadSnapPoint(g.u,g.v);cv.style.cursor='crosshair';}
  ...
  cv.addEventListener('pointerleave',()=>{cadDrawing=false;cadStart=null;cadCur=null;cadHover=null;cadBBoxP1=null;cadUpdateAll();});
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){
      ...
      else if(cadTool==='bbox'){cadBBoxP1=null;if(cadManualBBox){cadManualBBox=null;}cadUpdateAll();}
    }
  });
}
