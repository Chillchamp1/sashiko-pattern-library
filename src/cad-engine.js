...
// Split cadLines at off-grid intersections only. Grid-aligned intersections are left
// for the manual Split tool. Called after new geometry is added.
function cadSplitOffGrid(){
  const EPS=1e-5;
...
function cadInit(){
  ...
  cv.addEventListener('pointerdown',e=>{
    ...
    if(cadTool==='draw'){
      ...
      // Try to merge at intersection point
      const didMerge=cadMergeAllAt(cadCur[0],cadCur[1]);
      if(didMerge){
        cadHistory.push({l:JSON.parse(JSON.stringify(cadLines)),f:[...cadFamilies]});
  ...
  cv.addEventListener('pointerup',e=>{
    ...
    if(cadTool==='draw'&&cadDrawing&&cadStart&&cadCur){
      if(cadStart[0]!==cadCur[0]||cadStart[1]!==cadCur[1]){cadHistory.push({l:JSON.parse(JSON.stringify(cadLines)),f:[...cadFamilies]});cadLines.push({start:cadStart,end:cadCur});cadFamsLocked=false;cadFamSel=-1;cadSplitOffGrid();cadAutoExtendGrid();}
    }
