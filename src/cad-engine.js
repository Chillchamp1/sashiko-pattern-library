...
let cadManualBBox=null,cadBBoxState=0,cadBBoxP1=null,cadBBoxP2=null,cadBBoxWidth=0,cadBBoxCurScr=null;
// cadBBoxState: 0=idle, 1=dragging first edge, 2=stretching width
const CAD_BBOX_DIRS_G=[[1,0],[0,1],[1,1],[1,-1]]; // grid-space sensible edge directions
const CAD_MICRO=10;
...
function cadBBox(){
  if(cadManualBBox) return {minU:cadManualBBox.minU,maxU:cadManualBBox.maxU,minV:cadManualBBox.minV,maxV:cadManualBBox.maxV};
...
// ── Manual BBox helpers ──────────────────────────────────────────────────────
function _cadBBoxScreenDirs(){
  const iso=cadGridType==='isometric';
  const ts=cadTileSize, a=CAD_COS30*ts, b=CAD_SIN30*ts;
  if(iso) return [[a,b],[-a,b],[0,2*b],[2*a,0]];   // u, v, u+v(vert), u-v(horiz)
  return [[ts,0],[0,ts],[ts,ts],[ts,-ts]];            // horz, vert, diag, antidiag
}
function _cadSnapEdgeDir(sx,sy){
  const dirs=_cadBBoxScreenDirs();
  let best=0,bestD=-Infinity;
  for(let i=0;i<dirs.length;i++){
    const L=Math.hypot(dirs[i][0],dirs[i][1])||1;
    const dot=(sx*dirs[i][0]+sy*dirs[i][1])/L;
    if(dot>bestD){bestD=dot;best=i;}
  }
  return dirs[best];
}
function _cadSnapPointAlong(curX,curY,orgX,orgY,dirX,dirY){
  // Snap cursor to nearest grid point along screen direction (dirX,dirY) from origin
  const go=cadS2G(orgX,orgY,cadOX,cadOY,cadTileSize);
  const gc=cadS2G(curX,curY,cadOX,cadOY,cadTileSize);
  const gdu=gc.u-go.u, gdv=gc.v-go.v;
  const gd=cadS2G(cadOX+dirX,cadOY+dirY,cadOX,cadOY,cadTileSize);
  const gdx=gd.u, gdy=gd.v;
  const gLen=Math.hypot(gdx,gdy)||1;
  const proj=(gdu*gdx+gdv*gdy)/(gLen*gLen);
  const steps=Math.round(proj);
  const ru=go.u+steps*gdx, rv=go.v+steps*gdy;
  const rp=cadG2S(ru,rv,cadOX,cadOY,cadTileSize);
  return{rp, gu:ru, gv:rv};
}
function _cadBBoxRectFromEdge(edgeStartScr,edgeEndScr,width){
  const ex=edgeEndScr.x-edgeStartScr.x, ey=edgeEndScr.y-edgeStartScr.y;
  const eLen=Math.hypot(ex,ey)||1;
  const px=-ey/eLen, py=ex/eLen; // unit perpendicular in screen space
  const A={x:edgeStartScr.x,y:edgeStartScr.y};
  const B={x:edgeEndScr.x,y:edgeEndScr.y};
  const w=Math.abs(width)<2?2:width; // minimum width 2px so rect is always visible
  const C={x:edgeEndScr.x+px*w,y:edgeEndScr.y+py*w};
  const D={x:edgeStartScr.x+px*w,y:edgeStartScr.y+py*w};
  return [A,B,C,D];
}
function _cadScreenCornersToBBox(corners){
  // Convert 4 screen corners to grid-space bbox including p=u+v, q=u-v ranges
  let mnU=Infinity,mxU=-Infinity,mnV=Infinity,mxV=-Infinity;
  let mnP=Infinity,mxP=-Infinity,mnQ=Infinity,mxQ=-Infinity;
  corners.forEach(p=>{
    const g=cadS2G(p.x,p.y,cadOX,cadOY,cadTileSize);
    mnU=Math.min(mnU,g.u);mxU=Math.max(mxU,g.u);
    mnV=Math.min(mnV,g.v);mxV=Math.max(mxV,g.v);
    const pp=g.u+g.v, qq=g.u-g.v;
    mnP=Math.min(mnP,pp);mxP=Math.max(mxP,pp);
    mnQ=Math.min(mnQ,qq);mxQ=Math.max(mxQ,qq);
  });
  return{minU:mnU,maxU:mxU,minV:mnV,maxV:mxV, mnP,mxP,mnQ,mxQ};
}
...
function cadDrawWorkspace(){
  ...
  {const tc=cadMacro*CAD_MICRO;
   ...
   x.setLineDash([]);}
  // BBox preview during manual drawing
  if(cadTool==='bbox'){
    // State 1: dragging first edge
    if(cadBBoxState===1&&cadBBoxP1&&cadBBoxCurScr){
      const sx=cadBBoxCurScr.x-cadBBoxP1.x, sy=cadBBoxCurScr.y-cadBBoxP1.y;
      const [sdx,sdy]=_cadSnapEdgeDir(sx,sy);
      const snap=_cadSnapPointAlong(cadBBoxCurScr.x,cadBBoxCurScr.y,cadBBoxP1.x,cadBBoxP1.y,sdx,sdy);
      x.strokeStyle='rgba(100,200,255,0.6)';x.lineWidth=2;x.setLineDash([6,3]);
      x.beginPath();x.moveTo(cadBBoxP1.x,cadBBoxP1.y);x.lineTo(snap.rp.x,snap.rp.y);x.stroke();
      x.setLineDash([]);
      x.fillStyle='rgba(100,200,255,0.8)';
      x.beginPath();x.arc(cadBBoxP1.x,cadBBoxP1.y,5,0,Math.PI*2);x.fill();
      x.beginPath();x.arc(snap.rp.x,snap.rp.y,5,0,Math.PI*2);x.fill();
    }
    // State 2: stretching width
    else if(cadBBoxState===2&&cadBBoxP1&&cadBBoxP2){
      const rect=_cadBBoxRectFromEdge(cadBBoxP1,cadBBoxP2,cadBBoxWidth);
      x.strokeStyle='rgba(100,200,255,0.55)';x.lineWidth=1.5;x.setLineDash([5,3]);
      x.beginPath();x.moveTo(rect[0].x,rect[0].y);
      for(let i=1;i<4;i++)x.lineTo(rect[i].x,rect[i].y);
      x.closePath();x.stroke();
      x.setLineDash([]);x.fillStyle='rgba(100,200,255,0.06)';
      x.beginPath();x.moveTo(rect[0].x,rect[0].y);
      for(let i=1;i<4;i++)x.lineTo(rect[i].x,rect[i].y);
      x.closePath();x.fill();
    }
  ...
  if(bbox){
    ...
    x.fillStyle=cadManualBBox?'rgba(100,180,255,0.08)':'rgba(255,255,255,0.06)';
    x.strokeStyle=cadManualBBox?'rgba(80,200,255,0.85)':'rgba(255,240,140,0.65)';x.lineWidth=cadManualBBox?2:1.5;x.setLineDash(cadManualBBox?[2,2]:[4,3]);
    x.beginPath();
    if(!cadBBoxRotated){
      ...
      // Project endpoints onto 45°-rotated axes p=u+v, q=u-v; use manual bbox p/q if set
      let mnP,mxP,mnQ,mxQ;
      if(cadManualBBox&&cadManualBBox.mnP!==undefined){
        mnP=cadManualBBox.mnP;mxP=cadManualBBox.mxP;mnQ=cadManualBBox.mnQ;mxQ=cadManualBBox.mxQ;
      }else{
...
function cadDrawPattern(){
  ...
  if(!cadBBoxRotated){
    ...
    let mnP,mxP,mnQ,mxQ;
    if(cadManualBBox&&cadManualBBox.mnP!==undefined){
      mnP=cadManualBBox.mnP;mxP=cadManualBBox.mxP;
      mnQ=cadManualBBox.mnQ;mxQ=cadManualBBox.mxQ;
    }else{
...
window.cadUpdateSettings=function(){
  ...
  cadZoom=1;cadPanX=0;cadPanY=0;cadManualBBox=null;cadBBoxState=0;cadBBoxP1=null;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr=null;
...
window.cadSetTool=function(t){
  if(cadTool==='bbox'&&t==='bbox'&&cadManualBBox){cadManualBBox=null;cadBBoxState=0;cadBBoxP1=null;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr=null;cadUpdateAll();return;}
  cadTool=t;
...
window.cadClearManualBBox=function(){cadManualBBox=null;cadBBoxState=0;cadBBoxP1=null;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr=null;cadUpdateAll();};
...
function cadInit(){
  ...
  cv.addEventListener('pointerdown',e=>{
    ...
    else if(cadTool==='bbox'){
      if(cadBBoxState===0){
        // Start drawing edge: snap click to grid, store screen position
        const g=cadS2G(pos.x,pos.y,cadOX,cadOY,cadTileSize);
        const sp=cadSnapPoint(g.u,g.v);
        cadBBoxP1=cadG2S(sp[0],sp[1],cadOX,cadOY,cadTileSize);
        cadBBoxState=1;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr={x:pos.x,y:pos.y};
        cadCur=[sp[0],sp[1]];
      }else if(cadBBoxState===2){
        // Finalize rectangle: convert 4 screen corners to grid-space bbox
        const rect=_cadBBoxRectFromEdge(cadBBoxP1,cadBBoxP2,cadBBoxWidth);
        if(rect)cadManualBBox=_cadScreenCornersToBBox(rect);
        cadBBoxState=0;cadBBoxP1=null;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr=null;cadCur=null;
      }
    }
    cadUpdateAll();
  });
  cv.addEventListener('pointermove',e=>{
    ...
    if(cadTool==='draw'||cadTool==='arc')cadCur=cadSnapPoint(g.u,g.v);
      ...
      cadBBoxCurScr={x:pos.x,y:pos.y};
      if(cadBBoxState===1&&cadBBoxP1){
        const sx=pos.x-cadBBoxP1.x, sy=pos.y-cadBBoxP1.y;
        const [sdx,sdy]=_cadSnapEdgeDir(sx,sy);
        const snap=_cadSnapPointAlong(pos.x,pos.y,cadBBoxP1.x,cadBBoxP1.y,sdx,sdy);
        cadBBoxP2=snap.rp;cadCur=[snap.gu,snap.gv];
      }else if(cadBBoxState===2&&cadBBoxP2){
        const ex=cadBBoxP2.x-cadBBoxP1.x, ey=cadBBoxP2.y-cadBBoxP1.y;
        const eLen=Math.hypot(ex,ey)||1;
        const px=-ey/eLen, py=ex/eLen;
        // Snap cursor to grid point, then compute perpendicular width
        const g=cadS2G(pos.x,pos.y,cadOX,cadOY,cadTileSize);
        const sp=cadSnapPoint(g.u,g.v);
        const snp=cadG2S(sp[0],sp[1],cadOX,cadOY,cadTileSize);
        const dx=snp.x-cadBBoxP1.x, dy=snp.y-cadBBoxP1.y;
        cadBBoxWidth=px*dx+py*dy;
        cadCur=[sp[0],sp[1]];
      }
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){
      if(cadTool==='arc'){cadArcState=0;cadArcCenter=null;cadArcStart=null;cadArcLabel();cadUpdateAll();}
      else if(cadTool==='bbox'){cadBBoxState=0;cadBBoxP1=null;cadBBoxP2=null;cadBBoxWidth=0;cadBBoxCurScr=null;if(cadManualBBox){cadManualBBox=null;}cadCur=null;cadUpdateAll();}
    }
  });
}
